...
- Scope: Independent Tauri mobile shell for the Takos product.
- Repository kind: `client`
- Direct sibling dependencies: `mobile-kit`, `takos`
- Repository gate: `bun run check`
- Canonical docs: [README.md](README.md), [src-tauri/plugins/keystore/README.md](src-tauri/plugins/keystore/README.md), [src-tauri/plugins/mobile-push/README.md](src-tauri/plugins/mobile-push/README.md)

## Ownership

- Owns: Takos-specific Tauri mobile client UX / Product-native keystore, push, deep-link, and plugin wiring / Host-backed chat, agent, memory, and Capsule flows
- Does not own: Takos host creation or control-plane state / Product-neutral mobile primitives / Operator credentials or deployment authority
- Hazards: The shell requires sibling mobile-kit and Takos release-status inputs. / Credential restore fails closed when the OS store is unavailable. / Remote push and store release require provider and physical-device evidence.
